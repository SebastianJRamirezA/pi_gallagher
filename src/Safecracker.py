...
class Safecracker(Game):
    ...
    def update(self, dt: float) -> None:
        ...
        if self.turning:
            speed = settings.FINE_DIAL_SPEED if self.fine_control else settings.DIAL_SPEED
            previous = self.dial
    ...
    def _target_angle(self) -> float:
        return settings.COMBINATION[self.stage] * 3.6
    ...
    def _passed_target(self, previous: float, current: float, direction: int) -> bool:
        ...
        if direction > 0:
            ...
            from_target = (target - previous) % 360.0
        else:
    ...
    def _confirm(self) -> None:
        ...
        if self._target_distance() <= 1.5:
            ...
            self.stage += 1
            if self.stage == len(settings.COMBINATION):
                self.complete = True
    ...
    def render(self, surface: pygame.Surface) -> None:
        ...
        pygame.draw.rect(surface, settings.COLOR_PANEL, (26, 20, 428, 230), border_radius=4)
        pygame.draw.circle(surface, (67, 64, 58), center, 91)
        pygame.draw.circle(surface, (29, 30, 33), center, 82)
        pygame.draw.circle(surface, settings.COLOR_STEEL, center, 75, 2)
        ...
        for number in range(0, 100, 5):
            ...
            pygame.draw.line(surface, settings.COLOR_BRASS, start, end, 2 if number % 10 == 0 else 1)
        ...
        pygame.draw.line(surface, settings.COLOR_BRASS, center, needle, 3)
        pygame.draw.circle(surface, settings.COLOR_BRASS, center, 7)

        render_text(surface, "THE QUIET DIAL", settings.FONTS["large"], 30, 32, settings.COLOR_TEXT)
        render_text(surface, f"TUMBLER  {min(self.stage + 1, 3)} / 3", settings.FONTS["small"], 30, 72, settings.COLOR_MUTED)
        render_text(surface, f"{self._dial_number():02d}", settings.FONTS["medium"], center[0], 224, settings.COLOR_TEXT, center=True)
        render_text(surface, self.message, settings.FONTS["small"], 30, 238, settings.COLOR_DANGER if self.overshot else settings.COLOR_TEXT)
        ...
        pygame.draw.rect(surface, settings.COLOR_DANGER if stress > 0.8 else settings.COLOR_BRASS, (345, 72, int(92 * stress), 9))
        render_text(surface, "STRESS", settings.FONTS["small"], 345, 55, settings.COLOR_MUTED)
        render_text(surface, "A/D or arrows  rotate     SHIFT  fine     SPACE  set", settings.FONTS["small"], 30, 260, settings.COLOR_MUTED)
